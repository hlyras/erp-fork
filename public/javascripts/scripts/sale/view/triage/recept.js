...
Sale.recept.view.filter = (service_orders) => {
	...
	for(let i in service_orders) {
		...
		service_order_div.append(lib.element.create("div", { 
			...
			onclick: `Sale.recept.serviceOrder.show(${service_orders[i].id})`
		}, service_orders[i].id));
...
Sale.recept.view.show = (service_order) => {
	...
	service_order_sales_header.append(lib.element.create("div", { class: "mobile-box lucida-grande em08 bold b5-8 margin-top-5 center" }, "Cliente"));
	...
	for(let i in service_order.sales) {
		...
		service_order.sales[i].customer_name && service_order_sale.append(lib.element.create("div", { class: "mobile-box b5-8 lucida-grande bold center" }, `${service_order.sales[i].customer_name}`));
		!service_order.sales[i].customer_name && service_order.sales[i].customer_trademark && service_order_sale.append(lib.element.create("div", { class: "mobile-box b5-8 lucida-grande bold center" }, `${service_order.sales[i].customer_trademark}`));
		...
		service_order_sale.append(lib.element.create("div", {
			class: "mobile-box b8 container height-40 border-st center pointer",
			onclick: `Sale.recept.controller.checkin(${service_order.sales[i].sale_id}, this)`
		}));
	...
	service_order_div.append(lib.element.create("input", {
		type: "button",
		onclick: `Sale.recept.serviceOrder.confirm(${service_order.id})`,
		class: "mobile-box b1 lucida-grande em11 bold border-lg-st radius-5 padding-10 margin-top-10 margin-bottom-10 center pointer",
	...
	// service_order_div.append(lib.element.create("input", {
	// 	type: "button",
	// 	onclick: `Sale.recept.serviceOrder.cancel(${service_order.id})`,
