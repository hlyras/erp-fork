...
Sale.shipment.view.filter = (sales, box) => {
	...
	for (let i in sales) {
		...
		box == "sale-filter-div" && sale_div.append(lib.element.create("img", {
			...
			onclick: `Sale.shipment.serviceOrder.add(${sales[i].id})`
		}));

		box == "order-filter-div" && sale_div.append(lib.element.create("img", {
			src: "/images/icon/back-arrow-black.png",
			class: "mobile-box b9 center size-30 icon",
			onclick: `Sale.shipment.serviceOrder.remove(${sales[i].id})`
		}));
...
Sale.shipment.serviceOrder.render = () => {
	...
	so_box.append(lib.element.createInfo("mobile-box b3 lucida-grande bold padding-5", "Método de Envio", `${Sale.shipment.serviceOrder.method}`));
	so_box.append(lib.element.createInfo("mobile-box b3 lucida-grande bold padding-5", "Quantidade de pedidos", `${Sale.shipment.serviceOrder.orders.length}`));
