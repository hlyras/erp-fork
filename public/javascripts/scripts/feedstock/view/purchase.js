...
Feedstock.purchase.view.filter = (purchases, pagination) => {
	...
	if (purchases.length) {
		for (let i = pagination.page * pagination.pageSize; i < purchases.length && i < (pagination.page + 1) * pagination.pageSize; i++) {
			let purchase_div = lib.element.create("div", { class: "box b2 container padding-5 margin-top-5 border-explicit" });
...
Feedstock.purchase.view.show = (purchase) => {
	...
	purchase.feedstocks.forEach(feedstock => {
		...
		console.log(feedstock.price);

		feedstock_div.appendChild(lib.element.info("b4 bold", "Preço", "R$" + feedstock.price.toFixed(4)));
		feedstock_div.appendChild(lib.element.info("b4 bold", "Total", "R$" + (feedstock.amount * feedstock.price).toFixed(2)));
