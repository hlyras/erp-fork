...
Customer.mailer.view.filter = (customers, pagination) => {
	...
	for (let i = pagination.page * pagination.pageSize; i < customers.length && i < (pagination.page + 1) * pagination.pageSize; i++){
		...
		customer_div.appendChild(lib.element.icon('b10', 20, "https://spaces.jariomilitar.com/erp-images/icon/sendmail.png", `Customer.mailer.controller.send(${customers[i].id}, this)`));
