...
IncomeOriginView.filter = (categories, pagination) => {
	if (categories.length) {
		...
		for (let i = pagination.page * pagination.pageSize; i < categories.length && i < (pagination.page + 1) * pagination.pageSize; i++) {
			...
			html += "<div class='mobile-box b7 avant-garde bold center nowrap'>" + categories[i].id + "</div>";
			html += "<div class='mobile-box b4-7 padding-5 v-center'>" + categories[i].name + "</div>";
			html += "<div class='mobile-box b7 padding-5 margin-top-5 center'><img class='icon size-20' src='/images/icon/edit.png' onclick='IncomeOriginController.edit(" + categories[i].id + ")'></div>";
			html += "<div class='mobile-box b7 padding-5 margin-top-5 center'><img class='icon size-20' src='/images/icon/trash.png' onclick='IncomeOriginController.delete(" + categories[i].id + ")'></div>";
			html += "</div>";
		};
