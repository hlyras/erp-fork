...
const Conference = function () {
	...
	this.create = () => {
		...
		let { query, values } = lib.Query.save(obj, 'cms_wt_erp.product_conference');
...
Conference.filter = ({ props, inners, params, strict_params, order_params }) => {
	let { query, values } = new lib.Query().select().props(props).table("cms_wt_erp.product_conference product_conference")
