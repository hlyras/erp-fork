...
prospectController.mailer.filter = async (req, res) => {
	...
	let period = { key: "customer_prospect.mailer", start: prospect.periodStart, end: prospect.periodEnd };
	let params = { keys: [], values: [] };
	let strictParams = { keys: [], values: [] };
	
	// lib.Query.fillParam("customer_prospect.state", prospect.state, strictParams);
	lib.Query.fillParam("customer_prospect.brand", prospect.brand, params);
	lib.Query.fillParam("customer_prospect.manager", prospect.manager, params);
	lib.Query.fillParam("customer_prospect.email", prospect.email, params);
	lib.Query.fillParam("customer_prospect.status", prospect.status, params);
	lib.Query.fillParam("customer_prospect.mailer_sub", prospect.mailer_sub, strictParams);
...
prospectController.meeting.filter = async (req, res) => {
	...
	let period = { key: "customer_prospect.meeting", start: req.body.periodStart, end: req.body.periodEnd };
	let params = { keys: [], values: [] };
	let strictParams = { keys: [], values: [] };
